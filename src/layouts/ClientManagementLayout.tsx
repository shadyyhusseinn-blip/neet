...
import { Palette, Upload, Image, Settings, LogOut, ChevronLeft, X } from 'lucide-react';
import { useAuthStore } from '../stores/authStore';

const SIDEBAR_ITEMS = [
  { id: 'dashboard', label: 'الرئيسية', icon: Palette, path: '/admin/client-management/dashboard' },
  { id: 'appearance', label: 'المظهر العام', icon: Palette, path: '/admin/client-management/appearance' },
  { id: 'deliverables', label: 'تسليمات العملاء', icon: Upload, path: '/admin/client-management/deliverables' },
  { id: 'content', label: 'إدارة المحتوى', icon: Image, path: '/admin/client-management/content' },
];
...
export default function ClientManagementLayout() {
  ...
  useEffect(() => {
    if (!isLoggedIn) {
      navigate('/login');
    } else if (user?.role !== 'admin' && user?.role !== 'client-manager') {
  ...
  return (
    <div className="h-screen w-screen bg-[#050508] text-white font-['Cairo','Tajawal',sans-serif] overflow-hidden" dir="rtl">
      ...
      <div className="relative z-10 h-full flex">
        ...
        >
          {sidebarOpen ? <ChevronLeft size={24} /> : <Palette size={24} />}
        </motion.button>
